...
const Resolvers = {
  Query: {
    whoAmI: async (_: any, args: any, context: any) => {
      ...
      const targetUser = await Account.findById(context.user._id)
        .populate("seenContent")
        .exec();
      return targetUser;
    },
    ...
    getLessonsByLevel: async (_: any, args: any, context: any) => {
      const allLevels = await Lesson.find({ level: args.level })
        .populate("content")
        .exec();
      return allLevels;
    },
    ...
    getQuestionForStudentInALesson: async (_: any, args: any, context: any) => {
      ...
      if (orderedQuestions.length > 0) {
        ...
        const listOfCorrectQuestionsId = givenCorrectAnswers.map((answer) =>
          answer.question.toHexString()
        );
        // If not, we make an inverse intersection of the orderedQuestions
        const unansweredQuestions = orderedQuestions.filter(
          (question) => !listOfCorrectQuestionsId.includes(question.id)
        );
    ...
    getContentByLesson: async (_: any, args: any) => {
      const filteredContent = await TextContent.find({
        lesson: args.lesson,
      }).exec();
    ...
    getQuestionsForALesson: async (_: any, args: any) => {
      const targetQuestions = await Question.find({
        lesson: args.lesson,
      }).exec();
  ...
  Mutation: {
    ...
    addQuestionForALesson: async (_: any, args: any, context: any) => {
      ...
      if (targetLesson != null) {
        const newQuestion = new Question({
          ...
          type: args.type,
          hierarchy: targetLesson.questions
    ...
    updateLevel: async (_: any, args: any, context: any) => {
      ...
      const matchingFields = Object.keys(args).filter((value) =>
        Object.keys(Level.schema.paths).includes(value)
      );
    ...
    updateLesson: async (_: any, args: any, context: any) => {
      ...
      const matchingFields = Object.keys(args).filter((value) =>
        Object.keys(Lesson.schema.paths).includes(value)
      );
      const update: any = new Object({});
      for (const field of matchingFields) {
        update[field] = args[field];
    ...
    updateContent: async (_: any, args: any, context: any) => {
      ...
      const matchingFields = Object.keys(args).filter((value) =>
        Object.keys(TextContent.schema.paths).includes(value)
      );
      const update: any = new Object({});
      for (const field of matchingFields) {
        update[field] = args[field];
      }
      const updatedObject = await TextContent.findByIdAndUpdate(
        args.targetContent,
        update
      );
      return updatedObject;
    ...
    updateQuestion: async (_: any, args: any, context: any) => {
      ...
      const matchingFields = Object.keys(args).filter((value) =>
        Object.keys(Question.schema.paths).includes(value)
      );
      const update: any = new Object({});
      for (const field of matchingFields) {
        update[field] = args[field];
      }
      const updatedObject = await Question.findByIdAndUpdate(
        args.targetQuestion,
        update
      );
      return updatedObject;
      ...
      if (targetQuestion != null) {
        ...
        switch (targetQuestion.type) {
          ...
          case ANSWER_TYPES.MULTIPLE:
            isCorrect = targetQuestion.correctAnswer.every((answer: string) =>
